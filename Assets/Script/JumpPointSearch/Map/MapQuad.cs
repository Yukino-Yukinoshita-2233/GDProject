...
namespace PathFinding
{
    ...
    public class MapQuad : IMap
    {
        ...
        // float minRowPos(最小行数), float minColPos(最小列数), float maxRowPos, float maxColPos
        ...
        {
            float rowPos = _mapSize._minRowPos + (row) * _rowHeight; //_mapSize._maxRowPos - (row + 0.5f) * _rowHeight;
            float colPos = _mapSize._minColPos + (col) * _colWidth;
